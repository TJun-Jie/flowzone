...
const StressIndexSchema = z.object({
  ...
  time: z.date().describe('Time of the stress index measurement in ISO GMT-8'),
...
const winsSchema = z.object({
  ...
  date: z.date().describe('Date of the win in ISO GMT-8'),
...
const lossesSchema = z.object({
  ...
  date: z.date().describe('Date of the loss in ISO GMT-8'),
...
const ActionItemSchema = z.object({
  ...
  dueDate: z.date().describe('Due date of the action item in ISO GMT-8'),
  ...
  startTime: z.date().describe('Start time of the action item in ISO GMT-8'),
  endTime: z.date().describe('End time of the action item in ISO GMT-8'),
...
export const generateActionItems = action({
    ...
    handler: async (ctx, args) => {
      console.log("start");
      const prompt = `
        you are generating JSON test data according to a given JSON format.
      `;
      const system_message = `
        generate an example event given the JSON schema
      `;
